...
export function AdminLoginForm({ initialErrorCode }: AdminLoginFormProps) {
  ...
  // Trata o erro vindo pela URL (?error=...)
  useEffect(() => {
    if (!initialErrorCode) return;

    const errorType = initialErrorCode as ErrorType;
    ...
    if (errorType === "credenciais") {
      ...
      message =
        "Você não tem permissão para acessar o painel. Entre com uma conta de administrador.";
    } else if (errorType === "inativo") {
      message = "Este administrador está inativo.";
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Login social – mesmo visual do painel/login (para CLIENT) */}
