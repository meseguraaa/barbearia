...
function normalizeError(raw: string | null): ErrorType {
  if (!raw) return undefined;
  if (raw === "credenciais") return "credenciais";
  if (raw === "desconhecido") return "desconhecido";
  if (raw === "permissao") return "permissao";
  if (raw === "missing_company") return "missing_company";
  return "desconhecido";
}
...
export default function PainelLoginPage() {
  ...
  const resolvedError: ErrorType = normalizeError(searchParams?.get("error"));
  ...
  if (resolvedError === "credenciais") {
    errorMessage = "E-mail ou senha inválidos.";
  } else if (resolvedError === "desconhecido") {
    errorMessage = "Ocorreu um erro ao fazer login. Tente novamente.";
  } else if (resolvedError === "permissao") {
    errorMessage =
      "Você não tem permissão para acessar o painel. Entre com uma conta de barbeiro ou administrador.";
  } else if (resolvedError === "missing_company") {
    errorMessage =
      "Sua conta não está vinculada a nenhuma empresa. Peça para o administrador vincular sua conta (CompanyMember/AdminAccess).";
  }
